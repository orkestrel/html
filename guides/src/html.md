...
| `renderText`         | function | `(node: HTMLNode) => string`                                                                                                                               | Projects a node to structural plain text: block elements and `br` become line boundaries, other whitespace collapses, `script` / `style` bodies are excluded, `title` / `textarea` text is kept. Lossy on purpose — heading level, link destination, list marker, and table geometry are not representable in plain text and do not survive (§ [Text is the lossy projection](#text-is-the-lossy-projection)). |
...
There are two renderers, and they are not equivalent. `renderHTML` is the structure-preserving one: canonical, reparsable, and — on a distilled document — the smallest form that still says which line was a heading, where a link pointed, and which cells belonged to one row. `renderText` is the flat one. Its unit is the BLOCK, not the node — it breaks a line per block-level element, so a cell or a list item holding two paragraphs becomes two lines — and what it drops is worth naming instead of discovering:
...
| `table` / `tr` / `th` / `td` | One line per block inside each cell, in source order, with no row or column association |
| `pre` / `code`               | ONE line — no fence, no language, and every newline collapsed to a space                |
...
- [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) — the escaping, URL, attribute, and text leaves; `mergeText` / `collapseText` / `extractRegion` / `pruneDocument`; both renderers including the hand-built-AST refusals; and the AST fixpoint and canonical idempotence laws.
