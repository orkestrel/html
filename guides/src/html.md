...
The element vocabularies, allowlists, entity table, and depth bound every engine reads, from [`constants.ts`](../../src/core/constants.ts). Every set and map is frozen.

| Name                   | Kind  | Behavior                                                                                                                                                                                                                      |
| ---------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VOID_ELEMENTS`        | const | `ReadonlySet<string>` — the 13 elements that cannot have children (`area` … `wbr`). Voidness is DERIVED from the name here, never stored on a node, so the two can never disagree.                                            |
| `RAW_ELEMENTS`         | const | `ReadonlySet<string>` — `{script, style}`: content is one verbatim, undecoded `TextNode` up to the case-insensitive close tag, with no tag scanning inside.                                                                   |
| `LITERAL_ELEMENTS`     | const | `ReadonlySet<string>` — `{textarea, title}`: like raw text, but character references ARE decoded, so `<title>a &amp; b</title>` holds `a & b`.                                                                                |
| `BLOCK_ELEMENTS`       | const | `ReadonlySet<string>` — the structural elements; they are what implicitly closes an open `p` and what `renderText` treats as a line boundary.                                                                                 |
| `IMPLIED_CLOSERS`      | const | `ReadonlyMap<string, ReadonlySet<string>>` — for each element that may be left open, the start tags whose arrival closes it. `p` maps to `BLOCK_ELEMENTS` itself rather than a second copy, so the two cannot drift.          |
| `SAFE_ELEMENTS`        | const | `ReadonlySet<string>` — `sanitize`'s default element allowlist: the document vocabulary that survives unchanged. A SAFE element outside it is unwrapped, not dropped.                                                         |
| `SAFE_ATTRIBUTES`      | const | `ReadonlySet<string>` — `sanitize`'s default attribute allowlist: `alt cite class colspan dir height href lang rowspan span start title width`. Deliberately narrow — no `id`, no `style`, no handler, and no resource `src`. |
| `SAFE_URL_SCHEMES`     | const | `ReadonlySet<string>` — `{http, https, mailto, tel}`. A relative URL is always allowed; every unlisted scheme is refused.                                                                                                     |
| `URL_ATTRIBUTES`       | const | `ReadonlySet<string>` — `{action, cite, formaction, href, poster, src}`: the values `sanitize` decodes and scheme-checks, and `distill` resolves against `DistillOptions.base`.                                               |
| `UNSAFE_ELEMENTS`      | const | `ReadonlySet<string>` — the hard floor: subtrees removed WHOLE whatever the options say, because unwrapping them is what makes them dangerous (a `script` body is live markup the moment its wrapper disappears).             |
| `CONTENT_ELEMENTS`     | const | `ReadonlySet<string>` — `distill`'s default keep set: prose, headings, lists, tables, code, and the inline marks that carry meaning. Everything else safe melts into it.                                                      |
| `BOILERPLATE_ELEMENTS` | const | `ReadonlySet<string>` — `{aside, footer, header, menu, nav}`: `distill`'s default regions, removed WITH their children, because a navigation menu's link text is noise in every reading of the page.                          |
...
| `NAMED_ENTITIES`       | const | `ReadonlyMap<string, string>` — the HTML 4.01 named character references plus XML's `apos`. A `Map`, so a hostile name such as `&constructor;` can never reach an object prototype; an unlisted name stays literal.           |
...
| `isSafeURL`        | function | `(value: string, schemes?: ReadonlySet<string>) => boolean` | Whether an ALREADY-DECODED URL passes the protocol floor: relative is allowed, protocol-relative (`//`, `\\`, `/\`, `\/`) is not, and `javascript:` / `data:` / `vbscript:` / `file:` are refused whatever `schemes` says. |
| `isEmptyElement`   | function | `(element: ElementNode) => boolean`                         | Whether the element has no children — what `distill` reads before dropping an empty non-void element.                                                                                                                      |
...
Pure, total leaves from [`helpers.ts`](../../src/core/helpers.ts) — the functional core `HTML`'s engines compose and callers reach for directly on a bare node. Every one of them is independently testable and none of them throws: a hostile value degrades to `''`, to an empty list, or to the input unchanged.
...
| `escapeMarkdown`     | function | `(value: string) => string`                                                                                      | Backslash-escapes text that would otherwise reparse as markdown syntax — what keeps `renderMarkdown`'s literal text literal.                                                                                                         |
| `sanitizeURL`        | function | `(value: string, schemes: ReadonlySet<string>) => string`                                                        | Decodes references to a BOUNDED fixpoint, strips ASCII whitespace and control characters, then scheme-checks; returns `''` for anything unsafe, and fails CLOSED when the value still changes past the bound.                        |
...
| `sanitizeAttributes` | function | `(node: ElementNode, attributes: ReadonlySet<string>, schemes: ReadonlySet<string>) => readonly HTMLAttribute[]` | The attribute half of the sanitize floor: allowlist, plus the unconditional refusals (`on*`, `style`, `srcdoc`, `xmlns`, an unwritable name) and the URL check that REMOVES rather than empties.                                     |
...
| `renderText`         | function | `(node: HTMLNode) => string`                                                                                     | Projects a node to structural plain text: block elements and `br` become line boundaries, other whitespace collapses, `script` / `style` bodies are excluded, `title` / `textarea` text is kept.                                     |
| `renderMarkdown`     | function | `(node: HTMLNode) => string`                                                                                     | Projects the supported subset to markdown (§ [The markdown projection](#the-markdown-projection)); everything else unwraps to its children. The prompt-shaped end of the pipeline.                                                   |
...
**Not goals, stated plainly.** This is a pragmatic, total, bounded parser, not a conformant HTML5 tree constructor. It does not implement HTML5 insertion modes, the adoption agency algorithm, or foster parenting; it has no SVG/MathML namespace handling (foreign content is removed whole by `sanitize` instead); it decodes the HTML 4.01 named-reference set rather than the full HTML5 table; it reports no diagnostics, issues, or source positions; it ships no CSS selector engine and no streaming tokenizer; and `distill`'s hidden-content rule reads the `hidden` / `aria-hidden` attributes, never computed style. Each of those is a deliberate exclusion with a cheaper, testable substitute, not an unfinished edge.
...
- **`renderHTML` / `renderText` / `renderMarkdown`** — descent stops at the cap; deeper content is not serialized.
...
What roundtrips is the AST, NOT the input bytes. Three laws hold, each tested in [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) and [`tests/src/core/HTML.test.ts`](../../tests/src/core/HTML.test.ts):

1. **AST fixpoint.** For every document `d` that `parseDocument` produced, `parseDocument(renderHTML(d))` deep-equals `d`. Rendering and reparsing is the identity on parser output.
2. **Canonical idempotence.** `renderHTML(parseDocument(renderHTML(d))) === renderHTML(d)`. The serialization is canonical: writing it twice writes the same bytes.
...
`sanitize` is a security boundary, and its options are allowed to narrow it, redirect it, or widen the _vocabulary_ — never to open a hole. Each of `elements`, `attributes`, and `schemes` REPLACES its default set rather than extending it, and underneath all three sits a floor the options cannot lower:
...
`renderMarkdown` projects a deliberate SUBSET and unwraps everything else to its children, so an unrecognized element costs its tag, never its text:

| HTML                         | Markdown                                                                                             |
| ---------------------------- | ---------------------------------------------------------------------------------------------------- |
| `h1` … `h6`                  | `#` … `######`                                                                                       |
| `p`                          | A paragraph, blocks separated by one blank line                                                      |
| `ul` / `ol` / `li`           | `- ` / `N. ` items, nested items indented to their marker width; `ol` honors `start`                 |
| `blockquote`                 | `> `-prefixed lines                                                                                  |
| `pre` > `code`               | A fenced block, language taken from `class="language-…"`, fence widened past any backtick run inside |
| `code`                       | A backtick span, fence widened and padded as needed                                                  |
| `a` / `img`                  | `[text](href)` / `![alt](src)`, the URL re-checked through `sanitizeURL`                             |
| `strong` / `b`, `em` / `i`   | `**strong**`, `*emphasis*`                                                                           |
| `hr`, `br`                   | `---`, a two-space hard break                                                                        |
| `table` / `tr` / `th` / `td` | A GFM table with an alignment delimiter row                                                          |

Literal text is protected by `escapeMarkdown`, so prose that happens to contain `*`, `[`, or a leading `#` reparses as itself. The projection's output is proved by reparsing it with `@orkestrel/markdown` — a development-only dependency, never a runtime import — so "valid markdown" is a tested claim rather than an intention.
...
- [`tests/src/core/parsers.test.ts`](../../tests/src/core/parsers.test.ts) — every exported scanner, the whole recovery table row by row, entity decoding, and a hostile corpus proving totality, the no-adjacent-text invariant, and guard soundness on 100,000-tag floods.
...
- [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) — the escaping, URL, attribute, and text leaves; `mergeText` / `collapseText` / `extractRegion` / `pruneDocument`; the three renderers including the hand-built-AST refusals; and the AST fixpoint and canonical idempotence laws.
