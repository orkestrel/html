...
| `SAFE_ATTRIBUTES`      | const | `readonly string[]` — `sanitize`'s default attribute allowlist: `alt cite class colspan dir height href lang rowspan span start title width`. Deliberately narrow — no `id`, no `style`, no handler, and no resource `src`.                |
...
| `NAMED_ENTITIES`       | const | `Readonly<Record<string, string>>` — the HTML 4.01 named character references plus XML's `apos`. Lookup is own-property only (`Object.hasOwn`), so a hostile name such as `&constructor;` is not an entry; an unlisted name stays literal. |
...
One more thing worth stating out loud: the URL floor is re-implemented here on purpose, not shared. `@orkestrel/markdown`'s `sanitizeUrl` enforces the same lower floor — strip every codepoint ≤ `U+0020` and `U+007F`–`U+009F`, refuse any two-character protocol-relative prefix drawn from `/` and `\`, extract an ASCII scheme, enforce an allowlist, keep relative / anchor / scheme-less values — because a sanitizer belongs to the output context it defends, and these two contexts differ above that floor in three ways that are properties of the pipelines rather than drift. Its allowlist is FIXED and closed, so `javascript:` / `data:` / `vbscript:` / `file:` are refused by the allowlist itself and a hard-ban list there would be dead code; here `schemes` comes from the caller and REPLACES the default, so those four need the unwidenable refusal above, which no option can argue past. It needs no character-reference pass, because it escapes the surviving value itself, so `&#106;avascript:x` reaches the browser as literal text whose `:` never begins a scheme; here the sanitized value is re-serialized and can be reparsed, so references are decoded to a bounded fixpoint BEFORE the scheme is read, and an obfuscated `https&colon;&sol;&sol;host` survives decoded while an obfuscated `javascript` does not. And it escapes inside its own sanitizer, where the result is a finished `href` attribute value, while `sanitizeURL` returns the raw survivor and lets `renderHTML` encode it later — which is why a kept `"` here is not a hole.

Because there is no shared function to test once, the adversarial corpus is what the two packages share: the same vectors — control and whitespace splices, case variance, all four protocol-relative forms, kept relative/anchor/query URLs, refused schemes, entity obfuscation — are mirrored vector-for-vector and group-for-group in both test suites ([`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) here, and its counterpart there), so a vector missing from one list is missing from both, and each divergence above is pinned by a named test instead of an absent assertion.
