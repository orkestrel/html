...
| Name              | Kind     | Signature                                                                 | Behavior                                                                                                                                                                                                                                                             |
| ----------------- | -------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `parseDocument`   | function | `(html: string, spans?: Map<HTMLNode, HTMLSpan>) => HTMLDocument`         | Parses a page or fragment into an `HTMLDocument`. TOTAL — malformed markup recovers per the [recovery table](#the-parse-pipeline) instead of throwing — and guarantees no two adjacent text siblings. The optional map records provenance without changing the tree. |
| `parseHTMLSource` | function | `(html: string) => readonly [source: string, offsets: readonly number[]]` | Applies the parser's CRLF, carriage-return, and null rewrite and maps each normalized UTF-16 boundary to its original-input boundary.                                                                                                                                |
| `parseHTMLSpan`   | function | `(offsets: readonly number[], start: number, end: number) => HTMLSpan`    | Projects one normalized half-open region through the boundary map returned by `parseHTMLSource`.                                                                                                                                                                     |
...
| `scanRawText`        | function | `(html: string, offset: number, name: string, entities?: boolean, spans?: Map<HTMLNode, HTMLSpan>) => { node: TextNode, next: number, closed: boolean }`   | Scans to the case-insensitive matching close tag of a raw or literal element, optionally decoding references; reports whether a complete close was found and can record the exact source region of its constructed text node.                                                                                                                                                                                                                                                                    |
...
| `rewriteDocument`    | function | `(document: HTMLDocument, rewrite: HTMLRewriteHandler, derivations?: Map<HTMLNode, HTMLNode>) => HTMLDocument`                                             | The bottom-up, copy-on-write ONE-TO-ONE rewrite `map` delegates to; a subtree nothing changed keeps its reference, so an identity rewrite allocates nothing. The optional map records each rebuilt node's single source.                                                                                                                                                                                                                                                                         |
...
| `collapseText`       | function | `(children: readonly HTMLNode[], derivations?: Map<HTMLNode, HTMLNode>) => readonly HTMLNode[]`                                                            | Collapses whitespace runs inside each direct text child while KEEPING edge spaces, because the space between `<b>one</b>` and `<i>two</i>` is a word boundary, not decoration. The optional map records each one-source text rebuild.                                                                                                                                                                                                                                                            |
| `extractRegion`      | function | `(document: HTMLDocument, names: readonly string[], derivations?: Map<HTMLNode, HTMLNode>) => HTMLDocument`                                                | Re-roots a document at the sole occurrence of the first qualifying name; an absent or repeated name is ambiguous evidence and is skipped, and an unqualified document is returned unchanged. The optional map records the region that produced a new root.                                                                                                                                                                                                                                       |
| `pruneDocument`      | function | `(document: HTMLDocument, prune: HTMLPruneHandler, derivations?: Map<HTMLNode, HTMLNode>) => HTMLDocument`                                                 | The bottom-up ONE-TO-MANY spine both shaping engines share: each node arrives with its children already pruned and flattened, and returns the nodes that replace it. A node at the cap is handed NO children — safety over fidelity. The optional map records each one-source rebuild and leaves joins unrecorded.                                                                                                                                                                               |
...
const [normalized, offsets] = parseHTMLSource('A\r\n𝕏')
normalized // 'A\n𝕏'
parseHTMLSpan(offsets, 2, 4) // { start: 3, end: 5 }
