...
The full node shape and handle contract, from [`types.ts`](../src/core/types.ts). `category` is the discriminant every node carries; absence is always `undefined`, never a sentinel. A `Shape` cell holds an interface's members in braces, and a type alias's value. The node categories are worked through in § [The AST model](#the-ast-model), the two option interfaces in § [The sanitize floor](#the-sanitize-floor) and § [The distill pass](#the-distill-pass).

| Name                  | Kind      | Shape                                                                                  | Summary                                                                                                                                                                                                             |
| --------------------- | --------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `HTMLAttribute`       | interface | `{ name: string, value?: string }`                                                     | Represents one attribute of an `ElementNode` - its `name` and, when the source wrote one, its `value`.                                                                                                              |
...
| `ElementNode`         | interface | `{ category: 'element', name, attributes, children }`                                  | Represents an element - `<p>`, `<table>`, `<my-widget>`, or any other tag, known or custom.                                                                                                                         |
| `TextNode`            | interface | `{ category: 'text', value: string }`                                                  | Represents a run of character data - the leaf node, whose `value` is already-decoded text.                                                                                                                          |
| `CommentNode`         | interface | `{ category: 'comment', value: string }`                                               | Represents a comment - `<!-- … -->` - whose `value` is verbatim inner text, never decoded and never parsed as markup.                                                                                               |
| `DoctypeNode`         | interface | `{ category: 'doctype', name, public?, system? }`                                      | Represents a document type declaration - `<!DOCTYPE html>` and its legacy public/system forms.                                                                                                                      |
| `HTMLDocument`        | interface | `{ category: 'document', children: readonly HTMLNode[] }`                              | Represents the root of a parsed AST - the ordered children of the whole input, whether that input was a full page or one fragment. The value `HTMLInterface.document` holds.                                        |
...
| `HTMLSpan`            | interface | `{ start: number, end: number }`                                                       | Represents a half-open region of the original HTML input, measured in UTF-16 code units.                                                                                                                            |
...
| `HTMLOpenPosition`    | interface | `{ overflow: boolean, position: number }`                                              | Describes one open element occurrence located across the parser's represented and depth-overflow stacks.                                                                                                            |
| `HTMLScan`            | interface | `{ node: TNode, next: number }`                                                        | Describes the node and close boundary returned by a scanner that produces one leaf node.                                                                                                                            |
...
| `HTMLDerivation`      | type      | `readonly [value: T, derivations: ReadonlyMap<HTMLNode, HTMLNode \| undefined>]`       | Carries a derived value and the source of each rebuilt node.                                                                                                                                                        |
...
| `HTMLHandler`         | type      | `(node: TNode, children: readonly T[]) => T`                                           | Represents a fold handler for one node category - receives the node and its children ALREADY folded to `T`, and produces the node's own `T`. The building block of an `HTMLHandlerMap` table.                       |
| `HTMLHandlerMap`      | interface | `{ document, element, text, comment, doctype }`                                        | Represents the total fold table for `HTMLInterface.fold` - one `HTMLHandler` per node category, keyed by that category. Every key is required, because a fold is total over the AST: there is no node it may skip.  |
...
| `HTMLInterface`       | interface | `{ document, span, walk, find, filter, map, reduce, fold, stream, sanitize, distill }` | Represents a parsed HTML document: the typed `HTMLDocument` AST plus the query, rewrite, fold, and reduction operations over it.                                                                                    |
...
| `MAX_DEPTH`            | const | `64`                                                                                                                          | Names the recursion depth the parser, the guards, the traversals, the renderers, and both shaping engines honor before they stop descending.                                                                                         |
...
| `findOpenPosition`   | function | `(represented: ReadonlyMap<string, readonly number[]>, overflow: ReadonlyMap<string, readonly number[]>, name: string) => HTMLOpenPosition \| undefined`   | Finds the deepest open occurrence of an element name across the parser's two stacks.                                                                               |
...
The public methods of each behavioral interface — one table per type, keyed by its backticked name. The `readonly document` member is Surface-documented earlier, not listed here. The two shaping methods are worked through in § [The sanitize floor](#the-sanitize-floor) and § [The distill pass](#the-distill-pass).
