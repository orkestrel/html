...
Pure, total leaves from [`helpers.ts`](../src/core/helpers.ts) — the lexical scanners the document walk composes, and the functional core `HTML`'s engines compose and callers reach for directly on a bare node. Every one of them is independently testable and none of them throws: a hostile value degrades to `''`, to an empty list, or to the input unchanged.

| Name                 | Kind     | Signature                                                                                                                                                  | Behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `normalizeSource`    | function | `(html: string) => readonly [source: string, offsets: readonly number[]]`                                                                                  | Applies the parser's CRLF, carriage-return, and null rewrite and maps each normalized UTF-16 boundary to its original-input boundary.                                                                                                                                                                                                                                                                                                                                                            |
...
`parseProvenance(html)` normalizes line endings and `U+0000`, then walks the source once with an explicit open-element stack — no tree-construction insertion modes, no adoption agency, no foster parenting. Text runs to the next `<`; a `<` that cannot start markup is literal text; a start tag pushes, a close tag pops the nearest match, and `IMPLIED_CLOSERS` closes what the author left open. The implied-close search crosses intervening inline elements and stops when `IMPLIED_BARRIERS` names a scope container for the candidate. Raw- and literal-text elements swallow their body to the matching close tag. Past `MAX_DEPTH` the parser stops nesting and appends to the deepest allowed element, so parsing stays total and no text is lost. `parseDocument(html)` returns the document from that same walk.
