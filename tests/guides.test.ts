...
const GUIDE_SPEC = 'guides/html.md'
...
await new GuideCommand({
	...
	const { isObject, parseJSON } = await import('@orkestrel/contract')
	...
	const identity = requireValue(
		Object.keys(MODULES).find((specifier) => !specifier.startsWith('@src/')),
		'Missing package identity module',
	)
	...
	it('opens the README with the guide tagline', () => {
		...
		expect(Reflect.get(manifest, 'name')).toBe(identity)
	...
	for (const { entry, guide, source } of rows) {
		describe(`${entry.concept}`, () => {
			...
			// comparison and names both sides; converge the selected authority through the native
			// entry, never by weakening this assertion. It pairs an example only where a title is
			// present on both sides, so an untitled `@example` block is outside this case.
